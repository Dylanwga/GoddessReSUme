...
构建器统一中文东亚语言为 `zh-CN`，在段落中显式启用中文换行禁则、按词处理西文，并关闭行尾标点悬挂。具体映射以 [中文换行规则](style-spec.md#中文换行与续行) 为准。项目符号仍使用原生悬挂缩进；不插入硬换行。结构验证之外，渲染须核对标点完整保留、行首行尾禁则及中英文混排，不能将缺字归为“视觉正常”。
